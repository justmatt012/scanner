...
Write-Host "  ╚═╝  ╚═╝ ╚═════╝ ╚═════╝ ╚══════╝ ╚═════╝ ╚═╝  ╚═╝" -ForegroundColor Red
Write-Host ""
Write-Host "         ROBLOX FAIR-PLAY SCANNER  v1.0" -ForegroundColor White
...
foreach ($scanPath in $ScanPaths) {
    ...
    foreach ($fileName in $allInjectorFiles) {
        ...
        if (Test-Path $fullPath) {
            # Find which injector this belongs to
            $injName = ($KnownInjectors | Where-Object { $_.Files -contains $fileName } | Select-Object -First 1).Name
            $severity = ($KnownInjectors | Where-Object { $_.Files -contains $fileName } | Select-Object -First 1).Severity
...
if ($totalDanger -gt 0) {
    Write-Host "  VERDICT: ❌ CHEATS / ILLEGAL FLAGS DETECTED" -ForegroundColor Red
    Write-Host ""
    ...
    Write-Host "  VERDICT: ⚠️  WARNINGS FOUND — Review manually" -ForegroundColor Yellow
    Write-Host ""
    ...
    Write-Host "  VERDICT: ✅ CLEAN — No cheats or illegal flags detected" -ForegroundColor Green
    Write-Host ""
